...
EOF
    ...
    if [[ "$RESOLVE_VERSION" == 16.0* ]] || [[ "$RESOLVE_VERSION" == 16.1* ]]; then
...
process_17() {
    ...
    if [[ "$RESOLVE_VERSION" == 17.0* ]] || [[ "$RESOLVE_VERSION" == 17.1* ]]; then
        create_directory "${RESOLVE_BASE_DIR}"/bin/BlackmagicRawAPI/
        link_object ../libs/libBlackmagicRawAPI.so "${RESOLVE_BASE_DIR}"/bin/libBlackmagicRawAPI.so
        link_object ../../libs/libBlackmagicRawAPI.so "${RESOLVE_BASE_DIR}"/bin/BlackmagicRawAPI/libBlackmagicRawAPI.so
    fi
...
if [[ ! -e "$INSTALLER_ARCHIVE" ]] || [[ "$INSTALLER_ARCHIVE" != $(basename "${INSTALLER_ARCHIVE}") ]]; then
...
if [[ "${FILENAMEP1}" != "DaVinci" ]] || [[ "${FILENAMEP2}" != "Resolve" ]]; then
    echo "[ERROR] File does not look like a valid DaVinci Resolve installer archive"
    exit 1
fi
if [[ "${FILENAMEP5}" == "Linux.run" ]] && [[ "${FILENAMEP3}" == "Studio" ]]; then
...
elif [[ "$FILENAMEP4" == "Linux.run" ]]; then
...
if [[ -z "$CI_TEST" ]]; then
    ...
    if ! TMPDIR=./tmp fakeroot dpkg-deb -b "${DEB_DIR}" "${DEB_DIR}".deb; then
        ERRORS=$((ERRORS + 1))
    fi
